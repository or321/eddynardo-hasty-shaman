...
Camera.prototype = {
	toLayoutComponent: function(levelIndex) {
		var levelNumber = levelIndex + 1;
		return [[160,90,0,56,65,0,0,1,0.5,0.4923076927661896,0,0,[]],24,102,[[1],[levelNumber],[0],[0]],[[1]],[1,"Default",0,1]];
	}
};
var Chest = function() {
	this.height = 16;
...
Level.prototype = {
	...
	,wallBlocksLayer: function() {
		...
		return ["GroundCol",1,this.generateSID(),true,[255,255,255],true,1,1,1,false,false,1,0,0,wallBlocksInstances,[]];
	}
	,texturesLayer: function() {
		return ["Ground",2,this.generateSID(),true,[255,255,255],true,1,1,1,false,false,1,0,0,[[[0,0,0,320,192,0,0,1,0,0,0,0,[[0,1,1]],[40,24,this.texture_code]],12,78,[],[],[0,8,8,0,0,0,0,1]]],[]];
	}
	,chestsAndTilesLayer: function() {
		...
		instances = instances.concat(result);
		return ["UnderPlayer",3,this.generateSID(),true,[255,255,255],true,1,1,1,false,false,1,0,0,instances,[]];
	}
	,enemiesLayer: function() {
		...
		instances = instances.concat(result);
		return ["Enemies",4,this.generateSID(),true,[255,255,255],true,1,1,1,false,false,1,0,0,instances,[]];
	}
	,playerLayer: function() {
		return ["Player",5,this.generateSID(),true,[255,255,255],true,1,1,1,false,false,1,0,0,[this.player.toPlayerSpriteLayoutComponent(this.globalMultiplier),this.player.toPlayerHitboxLayoutComponent(this.globalMultiplier)],[]];
	}
	,overPlayerLayer: function() {
		return ["OverPlayer",6,this.generateSID(),true,[255,255,255],true,1,1,1,false,false,1,0,0,[],[]];
	}
	,uiLayer: function(levelIndex) {
		var instances = [this.camera.toLayoutComponent(levelIndex)];
		...
		return ["UI",7,this.generateSID(),true,[255,255,255],true,1,1,1,false,false,1,0,0,instances,[]];
	}
	...
	,toLevelLayoutData: function(levelIndex) {
		return ["Level" + (levelIndex + 1),320,180,true,"LevelCode",this.generateSID(),[this.backgroundLayer(),this.wallBlocksLayer(),this.texturesLayer(),this.chestsAndTilesLayer(),this.enemiesLayer(),this.playerLayer(),this.overPlayerLayer(),this.uiLayer(levelIndex),this.fullScreenFXLayer()],[],[]];
	}
};
var Main = function() { };
Main.__name__ = true;
...
Player.prototype = {
	toPlayerSpriteLayoutComponent: function(globalMultiplier) {
		return [[this.positionX,this.positionY,0,16 * this.widthMultiplier * globalMultiplier,24 * this.heightMultiplier * globalMultiplier,0,0,1,0.5,1,0,0,[]],8,1069,[],[[],[0,0,0.8,0.4,1]],[0,"Idle",0,1]];
	}
	,toPlayerHitboxLayoutComponent: function(globalMultiplier) {
		return [[this.positionX,this.positionY,0,10 * this.widthMultiplier * globalMultiplier,15 * this.heightMultiplier * globalMultiplier,0,0,0.5,0.5,1,0,0,[]],9,1321,[[0],[0],[0],[1],["No Spell"],[1],[40],[1],[0],[1],[0],[0]],[[80 * Math.pow(globalMultiplier,0.75),1500,1500,130 * Math.sqrt(globalMultiplier),350,1000,0,0,0,1]],[1,"Default",0,1]];
	}
};
...
Player.baseSpriteWidth = 16;
Player.baseSpriteHeight = 24;
Player.baseHitboxWidth = 10;
Player.baseHitboxHeight = 15;
Player.baseHorizontalSpeed = 80;
Player.baseJumpSpeed = 130;
Main.main();
